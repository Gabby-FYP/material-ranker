...
def send_email_verification_mail(user_id: UUID) -> None:
    ...
    with Session(engine) as session:
        ...
        context = {'user': user, 'verification_link': f'/accounts/verify-email/{email_token}'}
...
def send_password_reset_mail(user_id: UUID) -> None:
    ...
    with Session(engine) as session:
        ...
        context = {'user': user, 'reset_link': f'/accounts/reset-password/set-password/{reset_token}'}
